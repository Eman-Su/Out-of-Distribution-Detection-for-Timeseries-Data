...
def entropy(hist, bit_instead_of_nat=False):
    ...
    if h.sum()<=0 or (h<0).any():
        print(f"[entropy] WARNING, malformed/empty input {str(hist)}. Returning None.")
        return None
    h = h/h.sum()
...
def extract_feature_bulk(x_):
	with mp.pool.Pool(processes=5) as pool: #increase this from 5 if you have more processors
...
def extract_feature(x):
	features = []
	rp = RecurrencePlot()
	transformed = rp.transform(x[np.newaxis, :])
	feats, _ = histogram(transformed.squeeze())
	features.append(np.mean(feats))
	features.append(entropy(feats))
	features.append(kurtosis(feats))
	features.append(skew(feats))
	features.append(np.std(feats))
